...
calibrate = 1; %Fit calibration images 1 = Yes, 0 = No, load saved Q-matrices
         ...
         [n, m] %Show which rectangle we are working on (n = x, m = y)   
              ...
              for j = 1:px+1          %Loop over pixels in rectangle (j,k)
                   for k = 1:py+1
                        ...
                        %Define anonymous function for nlinfit
                        my_func = @(beta,x) (beta(1)*(((sin(beta(2)*x(1,:))+x(2,1)).^2))+beta(3));
